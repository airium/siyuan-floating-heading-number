...
浮动标题编号插件为思源桌面编辑器中的标题添加准确且可配置的层级编号。

* **基于完整文档精确编号。** 即使长文档仅渲染了一部分，编号仍保持准确。
...
* 编号根据内核返回的完整 BlockDOM 计算，因此即使长文档仅有一部分挂载在编辑器中，编号仍然准确。
...
* 可配置每个编号前后的文本，默认前缀为小节号（`§`），默认后缀为空。
* 当对应侧栏窄于配置的最小宽度时，自动隐藏位于标题外部的编号。
* 保留折叠标题控件和选中遮罩；选择内容时保持编号可见且不会将编号加入复制内容，标题悬停或侧栏操作期间隐藏外部编号，并在拖动块时隐藏编号。
...
以下分离 DOM 合成基准于 2026-07-22 使用 Node.js 24.15.0 和 happy-dom 20.10.6 记录。每个测试文档每十个块包含一个标题，渲染时挂载前 200 个标题，以模拟思源编辑器仅加载部分 DOM 的情况。
...
1. 在最新的 `main` 分支上运行 `pnpm version:set 0.4.0`，参数使用目标 `主版本.次版本.修订版本`。该命令会同时更新 `plugin.json` 和 `package.json`。
2. 在 `CHANGELOG.md` 中添加形如 `## v0.4.0 - YYYY-MM-DD` 的带日期章节；其中的内容会成为 GitHub Release 说明。
3. 运行 `pnpm check`，检查生成的 `package.zip`，然后提交清单和更新日志的变更并推送到 `main`。
4. 在 GitHub 打开 **Actions > Release > Run workflow**，选择 `main`，输入不带 `v` 前缀的 `0.4.0`。

工作流仅接受从 `main` 发起的发布，并会检查输入与两个清单中的版本是否一致、拒绝已存在的标签或 Release、使用锁定的依赖安装、运行完整的检查/构建/打包验证，以及要求存在匹配的更新日志。全部通过后，它会在触发工作流的提交上创建标签和 Release `v0.4.0`，并且仅附加 `package.zip`。请勿手动创建标签，也不要替换已有 Release 中的文件；任何修正都应发布新版本。
